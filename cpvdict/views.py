...
class ConstructionWorksClassificationListView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        try:
            ...
            sum_rb = {}
            for unit in units:
                orders = Order.objects.all().filter(genre__name_id='RB').filter(unit=unit).filter(
...
class ShowInfoWorkObject(LoginRequiredMixin, View):
    ...
    def get(self, request, id, year):
        try:
            ...
            orders = Order.objects.all().filter(date__year=year).filter(genre__name_id='RB').filter(
                unit__id=id).order_by(
                'date')
    ...
    def get(self, request, year):
        try:
            ...
            for unit in self.units:
                orders = Order.objects.all().filter(brakedown=False).filter(genre__name_id='RB').filter(
                    unit=unit).filter(
                    date__year=year)
                sum = 0
                for order in orders:
